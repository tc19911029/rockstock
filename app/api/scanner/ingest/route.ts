...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 2026-06-13：EODHD 不續訂 — 補缺直接走 scanner 預設鏈
    // （TW: FinMind→TWSE→Yahoo / CN: EastMoney→Tencent→Yahoo，MultiMarketProvider 單一事實）
    const report = await ensureCoverage(symbols, market, asOfDate, {
      fetchCandles: (symbol: string, date?: string) => scanner.fetchCandles(symbol, date),
      maxIngestCount: 300,
