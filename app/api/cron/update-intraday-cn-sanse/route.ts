/**
 * GET /api/cron/update-intraday-cn-sanse
 *
 * 三色資金（陸股自創策略）盤中即時掃描。把 L2 快照當作「今日未收的那根日K」
 * 合成到封存序列重算三色訊號，寫進獨立的 intraday 快照（不碰盤後 20:05 封存版）。
 *
 * 對比 scan-cn-sanse（盤後）：
 *   - 讀 L2 即時快照 → 合成今日進行中日K（盤後是讀純封存日K）
 *   - 寫 cn-sanse-scan-intraday/{date}.json（盤後寫 cn-sanse-scan/{date}.json）
...
export async function GET(req: NextRequest) {
  ...
  if (!force && !isMarketOpen('CN') && !isPostCloseWindow('CN')) {
    return apiOk({ skipped: true, reason: 'CN 非開盤時段也非盤後窗口' });
  }

  const date = getCurrentTradingDay('CN');
  ...
  try {
    ...
    const { scanSanSe } = await import('@/lib/cn-sanse/scan');
    const { saveSanSeIntraday } = await import('@/lib/cn-sanse/scanStorage');
    ...
    console.error('[update-intraday-cn-sanse] 掃描失敗:', err);
