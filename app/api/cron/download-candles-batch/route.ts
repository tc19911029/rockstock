...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if (snap && snap.quotes.length > 0 && snap.date === lastTradingDate) {
        ...
        for (const q of snap.quotes) {
          ...
          // 會撞 000001(平安銀行) key，且指數在 snapshot 排最後 → 指數值蓋進個股、寫進 L1（2026-06-02 實例）
            ...
            if (internalGaps.length > 0) {
              ...
              if (l2Quote) {
                ...
                if (suspectsLimitOverwrite(prevBar?.close, l2Quote, market, code)
                  ...
                  console.warn(
                    `[download-batch] ${symbol} ${lastTradingDate} L2 close 異常(漲跌停/單日>50%偏離=疑撞庫壞抓)，` +
    ...
    if (batch === 1) {
      ...
      // 大盤代理（對齊 *Scanner.getMarketTrend 實際讀的 symbol）：
      // - TW: ^TWII（加權指數）— 之前下載 0050.TW 但沒人讀，^TWII 又只在 scanner 首次 miss 時 self-warm
      // - CN: 000001.SS（上證指數）— 之前下載 000300.SS（滬深300）對應 source 已切換
      const proxySymbols = market === 'TW'
        ? ['^TWII', ...ACTIVE_ETF_SYMBOLS]
        : ['000001.SS'];
