...
export async function POST(req: NextRequest) {
  ...
  const symbol = (bodyJson.symbol ?? '').trim();
  ...
  if (bodyJson.action === 'remove') {
    ...
    roster.entries = roster.entries.filter(e => e.symbol !== symbol);
    if (roster.entries.length === before) return apiError('symbol not in roster', 404);
    roster.updatedAt = new Date().toISOString();
    await saveLockRoster(roster);
    return apiOk({ removed: symbol, total: roster.entries.length });
  }
  ...
  if (bodyJson.action === 'add') {
    ...
    roster.entries.push({
      symbol, name: bodyJson.name ?? symbol, market,
