...
export default function DisclaimerPage() {
  return (
    <div className="min-h-screen bg-background text-foreground p-6 max-w-3xl mx-auto">
      <Link href="/" className="text-sm text-muted-foreground hover:text-foreground mb-6 inline-block">&larr; 返回主頁</Link>

      <h1 className="text-2xl font-bold mb-6">免責聲明與使用條款</h1>

      <div className="space-y-6 text-sm text-foreground/80 leading-relaxed">
        <section>
          <h2 className="text-lg font-bold text-foreground mb-2">投資風險警告</h2>
        ...
        <section>
          <h2 className="text-lg font-bold text-foreground mb-2">回測結果限制</h2>
        ...
        <section>
          ...
          <div className="bg-card/80 ring-1 ring-foreground/10 rounded-xl p-4 space-y-2">
            ...
            <ul className="list-disc list-inside space-y-1.5 text-foreground/80">
              ...
              <li><strong>Walk-Forward 穩健度</strong>：若低於 <strong className="text-foreground">70%</strong>，表示策略可能<strong>過度擬合</strong>歷史數據，實際交易表現可能大幅不如回測</li>
              <li><strong>覆蓋率</strong>：低於 90% 表示部分股票資料缺失，回測結果存在<strong>倖存者偏差</strong>，實際報酬可能比顯示的低 10-20%</li>
              <li><strong>實際 vs 回測差距</strong>：一般而言，實際交易績效會比回測結果差 <strong className="text-foreground">20-30%</strong>，主要來自滑點、流動性、心理因素等</li>
            </ul>
            <p className="text-xs text-muted-foreground mt-2">建議：先以小資金紙上模擬交易至少 1-3 個月，確認策略在實盤環境中可行後，再逐步投入資金。</p>
          </div>
        </section>

        <section>
          <h2 className="text-lg font-bold text-foreground mb-2">當沖提示功能</h2>
        ...
        <section>
          <h2 className="text-lg font-bold text-foreground mb-2">責任限制</h2>
        ...
        <section>
          <h2 className="text-lg font-bold text-foreground mb-2">隱私政策</h2>
        ...
        <section>
          <h2 className="text-lg font-bold text-foreground mb-2">智慧財產權</h2>
          <p>本軟體參考朱家泓老師的技術分析方法論作為教學基礎，但軟體本身及其程式碼為獨立開發。</p>
        </section>

        <section>
          <h2 className="text-lg font-bold text-foreground mb-3">常見問題 FAQ</h2>
        ...
        <div className="border-t border-border pt-4 mt-8 text-xs text-muted-foreground">
          <p>最後更新：2026 年 3 月</p>
