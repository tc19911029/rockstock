...
**2026-04-20 命名重整**：F→E、E→D（移除 F，字母連續）

| 策略 | 名稱 | 書本核心 | 實作 |
|---|---|---|---|
| **A** | 六條件 + 高勝率6位置 + 十大戒律 + 淘汰法 | 寶典 p.54/p.57-58/Part10/Part12 | `trendAnalysis.ts` + `highWinRateEntry.ts` + `entryProhibitions.ts` + `eliminationFilter.ts` |
| **B** | 盤整突破 + 回後買上漲 | 《5步驟》p.40 位置 1/2 | `breakoutEntry.ts` |
| **C** | V 形反轉 | 寶典 Part 12 祕笈圖#1 + 《5步驟》位置 6 + 《抓住飆股》p.316 | `vReversalDetector.ts detectVReversal` |
| **D** | 缺口（跳空上漲）| 《5步驟》p.40 位置 4 | `gapEntry.ts detectStrategyD` |
| **E** | 一字底突破（獨立版）| 《抓住飆股》25 型態 #9 | `highWinRateEntry.ts detectStrategyE` |

### 二、走圖輔助信號（2 種，不影響進出場）

| 信號 | 書本 | 實作 |
|---|---|---|
| **K 線型態**（F）| 寶典 Part 3 p.199-245（變盤線/反轉/繼續 3 類全實作）| `klinePatterns.ts` |
...
**書本**：
- 盤整突破：《做對5步驟》位置 1
- 回後買上漲：寶典 p.238-244 波浪型態戰法（「底底高 + 收盤站上MA5 → 買進」+ 9點提醒第2條）
...
| **pullback_buy 回後買上漲** | 多頭趨勢 + 近期曾跌破MA5（有回後）+ 今日站回MA5 + 紅K + 量 + **突破前根K高點** | `detectTrend === '多頭'` + 近20根內 `close < MA5` + `close > MA5` + 紅K實體≥2.5% + 量≥前日×1.3 + `close > prev.high` | ✅ 100% |
...
**書本**：朱家泓《K 線交易法》V 形反轉 4 條件 + 寶典 Part 12 祕笈圖#1「低檔大量長紅K反轉」+《5步驟》位置 6 +《抓住飆股》p.316
...
2. **變盤線止跌**：過去 1-15 根內出現變盤線（十字 / 紡錘 / 長下影），量能不限
3. **止跌等待**：變盤線之後到今日前，最低不跌破變盤線 low（可等數天）
...
**實作**：[lib/analysis/highWinRateEntry.ts:100](../lib/analysis/highWinRateEntry.ts) `detectStrategyE`（原 `detectFlatBottom` 保留 alias）
...
**注意**：E 策略 ≠ 高勝率位置 4。位置 4 只要「糾結 + 紅K 突破 + 大量」；E 策略加 40 天/量縮等詳版規則。
...
| 概念 | 倍數 | 用途 | 出處 |
|---|---|---|---|
| **攻擊量（進場）** | 前日 × **1.3** | 六條件 ④、戒律 9、V 反轉、B 策略、D 策略共同扳機 | 寶典 p.54 ④ |
| **爆量（主力動作）** | 前日 × **2** | R9「爆量不漲」、E 一字底突破量、V 反轉爆量 | 《抓住飆股》+ 理財達人秀 YouTube #17 |
...
- ~~MA 開口 10%~~（戒律 3）
- ~~近 10 天跌破 MA5~~（B pullback_buy，改為近 20 根內任一根）
- ~~回檔 3-15 天 / ≥3%~~（B pullback_buy）
- ~~KD 金叉~~（B pullback_buy，無書本出處，2026-04-21 移除）
- ~~10 根盤整 / 15%~~（B 盤整突破）
- ~~40 天要求~~（高勝率位置 4）
- ~~R3/R8/R10/R11~~
- ~~戒律 11（上升切線）~~
