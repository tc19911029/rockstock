...
> **本次更新（2026-05-22）**：
> 1. 整體擴展涵蓋 v12 字母軌道（J/K/L/M/N/O/P/Q/R 9 個新字母）、軌道架構（多頭軌/反轉軌/戰法軌/機械軌）、Tier 1 三項對齊、Phase C 過濾、停損停利 v12。原始基礎為 2026-04-21 整理。
> 2. 三項偏離書本項目於 2026-05-22 審計後已回滾為書本原文：HIGH_DEVIATION_PCT 25% → **15%**（書本 p.568）、淘汰法「警示不擋」→ **hard gate**（書本「立即出場」）、KD 向下「flag 保留不擋」→ **hard gate**（書本短線守則 #9）。詳見「附錄 D.1」。
> 3. Q 戰法（MA3/10/24）經書本 p.262 OCR 確認從「資料不足」確認為「書本明確」。
> 4. `chipDivergence.ts` 重新對齊為書本 p.57「量價背離」（不是法人籌碼），僅供走圖頂部 banner 顯示，不進入選股流程。
>
> 完整 stale commit 清單與審計方法見 [docs/audit/2026-05-22-knowledge-contamination-audit.md](audit/2026-05-22-knowledge-contamination-audit.md)；淘汰法回滾影響量測見 [docs/audit/2026-05-22-elimination-hard-gate-impact.md](audit/2026-05-22-elimination-hard-gate-impact.md)。
...
| ⑥-4 虧 10% 強制出場 | — | 同上 | ✅ |
| ⑥-5 結構破強制出場 | — | 同上 | ✅ |
| ⑤-② 切 MA5 | MA20 乖離 > `HIGH_DEVIATION_PCT` | 寶典 p.568 | ✅ 2026-05-22 已回滾為書本 15% |
...
| **線上課 CH1-3**（PDF 73 頁 + 影音）| Tier 1 三項對齊（朱老師 CH3 均線 / 林穎 CH2 K線 / 朱老師 CH3 接近壓力區）— ✅ **2026-05-22 筆記已固化**，索引見附錄 F |
...
- 六條件 ①-⑥（含 ⑥ KD 向下 hard gate，2026-05-22 已回滾對齊書本短線守則 #9）
- 十大戒律 1-10（戒律 3 乖離門檻 2026-05-22 已從 25% 回滾為書本 15%；戒律 11 已移除）
- 淘汰 R1/R2/R5/R6/R7/R9（執行方式 2026-05-22 已從「警示不擋」回滾為書本「立即出場」hard gate；R3/R8/R10/R11 已移除）
- 高勝率位置 1/2/3/5/6
- 字母 B 回後買上漲
- 字母 C 盤整突破
- 字母 E 缺口
...
- ~~字母 Q MA3/10/24~~ → ✅ **2026-05-22 已排除**：經書本 p.262 OCR 確認對齊書本
- Tier 1 三項對齊（朱老師 CH3 / 林穎 CH2，課程筆記未固化）
- 再進場規則（「書本戰法 1 波浪 / 戰法 4 二條均線」，無頁碼）

## ✅ 已回滾的偏離（2026-05-22）

以下三項曾於 2026-05-20 commit 496309e 被改為偏離書本，2026-05-22 審計後已全部回滾為書本原文。完整回滾說明見「附錄 D.1」：

- HIGH_DEVIATION_PCT 25% → **15%**（書本 p.568）
- 淘汰法「警示不擋」→ **hard gate**（書本「立即出場」）
- KD 向下「flag 保留不擋」→ **hard gate**（書本短線守則 #9）
...
這些規則依據「網路課程」「Podcast」「YouTube」「線上課 CH1-3」，沒有完整書本對照。

## E.1 ~~Q 戰法三均線~~ — ✅ 2026-05-22 已排除

> **狀態變更**：2026-05-22 二次審計從《抓住線圖股民變股神》p.262 OCR 確認原文「採用3日均線及10日均線為操作進出依據。24日均線做為趨勢判定」，Q 戰法 MA3/10/24 與書本完全一致。p37「第三大金剛 MA5/10/20/60」是入門均線教學體系，與「三均線戰法 MA3/10/24」屬不同戰法/教學階段，無衝突。本項已從第四類移入第一類（書本明確）。
...
**待辦**：
- ~~T6: 整理線上課 CH1（朱老師：趨勢）筆記~~ → ✅ 2026-05-22 已完成（本機 `~/Desktop/朱家泓課程/筆記/CH1-趨勢/` 9 檔，見附錄 F）
- ~~T7: 整理線上課 CH2（林穎：K 線）筆記~~ → ✅ 2026-05-22 已完成（本機 `~/Desktop/朱家泓課程/筆記/CH2-K線/` 9 檔）
- ~~T8: 整理線上課 CH3（朱老師：均線）筆記~~ → ✅ 2026-05-22 已完成（本機 `~/Desktop/朱家泓課程/筆記/CH3-均線/` 6 檔）
- T9: 對齊 v12 缺口（8 項真缺漏 + 5 項參數不對齊），追蹤清單 → [docs/audit/v12-gaps-from-online-course.md](audit/v12-gaps-from-online-course.md)
...
# 附錄 F：線上課程素材索引（個人 local 資料）

朱家泓「技術分析全攻略」線上課程已完整整理筆記，位置在 `~/Desktop/朱家泓課程/`（**不在 git repo**，純個人本機素材）。
本附錄只索引筆記位置與課程口訣摘要，**不複製課程內容**到本 repo（版權考量）。

**課程進度**：CH1（朱老師：趨勢）9/9 ✅、CH2（林穎：K 線）9/9 ✅、CH3（朱老師：均線）6/6 ✅、CH4+（成交量等）⏳ 待上線。

## F.1 筆記目錄

### CH1 趨勢（朱家泓親講）

- `~/Desktop/朱家泓課程/筆記/CH1-趨勢/00-章節總覽.md`
- `01-轉折波與股票的3大走向.md`
- `02-如何辨別多頭的股票.md`
- `03-如何辨別空頭的股票.md`
- `04-如何辨別盤整的股票.md`
...
- `~/Desktop/朱家泓課程/筆記/CH2-K線/00-章節總覽.md`
- `01-K線起源與基本概念.md`
- `02-K線本身的支撐壓力與強弱的改變.md`
- `03-K線橫盤的原則與進出場的確認.md`
- `04-實體長紅棒.md`
- `05-實體長黑棒.md`
- `06-高檔變盤線.md`
- `07-低檔變盤線.md`
- `08-三根K棒組合.md`
- `09-K線4元素的進階意義與1比2價的應用.md`

### CH3 均線（朱家泓親講）

- `~/Desktop/朱家泓課程/筆記/CH3-均線/00-章節總覽.md`
- `01-均線的基本概念.md`
- `02-移動扣抵.md`
- `03-均線三大力量.md`
- `04-多頭排列與空頭排列.md`
- `05-黃金交叉、死亡交叉與均線糾結.md`
- `06-葛蘭碧8大法則.md`

### 跨章節主題

- `~/Desktop/朱家泓課程/筆記/00-總綱.md`（五口訣、四大金剛、三大趨勢鐵則、與 rockstock codebase 對應索引、8 個書本 vs rockstock 缺口）
- `~/Desktop/朱家泓課程/筆記/98-課程vs書本差異.md`（CH1-3 vs 6 本書詳細對照）
- `~/Desktop/朱家泓課程/筆記/99-V12差異報告.md`（rockstock v12 vs 課程的 40 項審查 + Tier 1/Tier 2 行動建議）

## F.2 課程關鍵口訣（從 00-總綱.md 摘錄）
...
| 行進間 | 續攻 / 續抱 | 續跌 / 續抱空單 |
| 高檔（漲一倍）| 出貨警訊（跌破 5 均出）| — |
| 低檔（跌一半）| — | 止跌訊號（爆量長黑、紅 K 確認）|

→ rockstock 對應：`lib/analysis/trendAnalysis.ts` `TrendPosition` 型別（目前只有「多頭上升段/末升段/空頭下跌段/末跌段/盤整」，無「接近壓力區」細分標籤，2026-05-21 commit 669f273 已部分補入）。

## F.3 課程 vs 書本差異（主要點，詳見 98-課程vs書本差異.md）

| 議題 | 書本 | 課程 | rockstock 現況 |
|---|---|---|---|
| 長紅實體棒閾值 | 寶典 p51-100「進場 K 線」**>2%** | CH2-4「絕對多方最強」**≥6.5%** | code 用 2%（`ruleUtils.ts:10` `isLongRedCandle()`），與「進場標準」對齊。**「長紅」vs「進場 K 線」可能是兩個概念**，課程更嚴格 |
| 長黑實體棒閾值 | 同上 | 同上 | code 統一 2% |
| 中棒/小棒分級 | 寶典模糊（常用「中長紅」）| 課程明確 6.5% / 3.5% / <3.5% | code 僅小/中長二分（缺 3.5-6.5% 中等級） |
| 葛蘭碧乖離 | ±15%（朱老師標準） | ±15%（**完全同**） | 買 4 `-10%`、賣 8 `+15%`（`granvilleRules.ts:173,324`），**不對稱且買方偏寬** |
| 「左藏紅右藏黑」 | 書本叫「高檔紅 K+黑 K 組合」 | **林穎新術語**（CH2-6） | code 高檔組合已實作（覆蓋/吞噬/夜星等），無此簡稱 |
| 「左長黑右長紅」 | 書本叫「低檔黑 K+紅 K 組合」 | **林穎新術語**（CH2-7） | code 低檔組合已實作（晨星/母子/雙星），無此簡稱 |
| 次日確認鐵律 | 提過但不執行化 | 反覆強調「變盤線次日才算」 | code 無「次日確認延遲機制」 detector（v12 缺口 C10） |
| 位置決定意義 | 隱含於各章 | 系統化、表格化（7 個位置） | code 有基本標籤，但「接近壓力區」細分 2026-05-21 commit 669f273 才補 |

## F.4 與 v12 缺口追蹤

完整 v12 vs 課程審查（40 項）見：[docs/audit/v12-gaps-from-online-course.md](audit/v12-gaps-from-online-course.md)
...
| 類別 | 路徑 |
|---|---|
| 原始 mp4 + PDF | `~/Desktop/朱家泓課程/線上課程/CH{1,2,3}/` |
| PDF 文字抽出 | `~/Desktop/朱家泓課程/raw/pdf/` |
| 影片轉錄 txt + srt | `~/Desktop/朱家泓課程/raw/transcript/` |
| 抽出的 mp3 音檔 | `~/Desktop/朱家泓課程/raw/audio/` |
| 自動化腳本 | `~/Desktop/朱家泓課程/scripts/extract_pdf.py`、`transcribe_video.py`（兩支都會跳過已處理檔案，CH4+ 上線時重跑即可增量） |
...
| 日期 | 修改 | 記憶檔 / commit |
|---|---|---|
| 2026-05-22 | **附錄 F 新增**：線上課 CH1-3 筆記索引固化（個人本機 `~/Desktop/朱家泓課程/筆記/` 24 章 + 課程vs書本差異 + V12 差異報告）；附錄 A 線上課欄位改標「✅ 筆記已固化」；附錄 E.2 T6/T7/T8 待辦標記完成；新增 v12 缺口追蹤清單 [docs/audit/v12-gaps-from-online-course.md](audit/v12-gaps-from-online-course.md) | 本次更新（依用戶整理筆記 `~/Desktop/朱家泓課程/筆記/99-V12差異報告.md`）|
| 2026-05-22 | **附錄 D 拆分為 D.1 已回滾 / D.2 仍偏離**：HIGH_DEVIATION_PCT 25%→15%、淘汰法 hard gate、KD 向下 hard gate 三項全數回滾為書本原文；Q 戰法書本依據從「資料不足」確認為「書本明確」（p.262 OCR）；chipDivergence 重命名為「量價背離」對齊書本 p.57；附錄 C 加入「已回滾的偏離」與「✅ 已排除」標記 | 本次更新（依 [docs/audit/2026-05-22-knowledge-contamination-audit.md](audit/2026-05-22-knowledge-contamination-audit.md) + [docs/audit/2026-05-22-elimination-hard-gate-impact.md](audit/2026-05-22-elimination-hard-gate-impact.md)）|
| 2026-05-22 | **全文擴展**：補齊 v12 字母軌道（J/K/L/M/N/O/P/Q/R）+ 軌道架構 + 第五類偏離項標紅 + 資料不足章節 + Tier 1 對齊 + Phase C + 停損停利 v12 | 初版擴展 |
