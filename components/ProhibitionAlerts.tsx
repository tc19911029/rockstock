...
function ProhibitionSection({
  ...
  const okColor   = direction === 'long' ? 'text-red-400'   : 'text-green-400';
  ...
  const warnBadge = direction === 'long' ? 'bg-red-600 text-white' : 'bg-green-700 text-white';

  if (!prohibited) {
    return (
      <div className="flex items-center gap-2 py-1">
        <span className={`text-xs font-medium ${okColor}`}>✅</span>
        <span className="text-xs text-slate-400">{title} — 無戒律觸發</span>
      </div>
    );
  }
  ...
  return (
    <div className={`border rounded overflow-hidden ${warnBg}`}>
      ...
      {expanded && (
        <div className="px-3 pb-2 border-t border-slate-700/40 space-y-1">
          {reasons.map((r, i) => (
...
function ShortConditionsSection() {
  ...
  const scoreColor = ready ? 'text-green-400' : score >= 3 ? 'text-yellow-500' : 'text-slate-500';
  ...
  return (
    <div className="border border-slate-700/50 rounded overflow-hidden">
      <div
        className="flex items-center gap-2 px-3 py-2 cursor-pointer select-none bg-slate-800/40"
        onClick={() => setExpanded(v => !v)}
      >
        <span className="text-xs text-slate-500">做空六條件</span>
        <span className={`text-sm font-bold ${scoreColor}`}>{score}/6</span>
        {ready && <span className="text-xs bg-green-700 text-white px-1.5 py-0.5 rounded font-bold">可放空</span>}
        {!ready && score >= 3 && <span className="text-xs text-yellow-500">待觀察</span>}
        <span className="text-slate-500 text-xs ml-auto">{expanded ? '▲' : '▼'}</span>
      </div>
      {expanded && (
        <div className="px-3 pb-2 pt-1 space-y-1 bg-slate-900/20">
          {conditions.map((c, i) => (
            <div key={i} className="flex items-center gap-2">
              <span className={c.pass ? 'text-green-400 text-xs' : 'text-slate-600 text-xs'}>
                {c.pass ? '✅' : '○'}
              </span>
          ...
          {!ready && (
            <p className="text-xs text-slate-600 pt-1">前5條未全過 → 不可進場做空</p>
          )}
        </div>
      )}
    </div>
...
export default function ProhibitionAlerts() {
  ...
  return (
    <div className="bg-slate-800 rounded-lg p-4 space-y-3">
      <h2 className="text-sm font-semibold text-slate-300">進場10大戒律</h2>
