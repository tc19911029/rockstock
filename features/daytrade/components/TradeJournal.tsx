  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
      <div className="px-4 py-2.5 border-b border-slate-800 bg-slate-800/40 flex items-center justify-between">
        <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">交易日誌</span>
        <div className="flex items-center gap-2">
          {entries.length > 0 && (
            <button onClick={exportCsv} className="text-[10px] text-slate-500 hover:text-slate-300 px-2 py-1 rounded border border-slate-700 hover:bg-slate-800">
              匯出 CSV
            </button>
          )}
          <button
      ...
      {stats && (
        <div className="grid grid-cols-4 border-b border-slate-800/60 divide-x divide-slate-800/60">
          {[
            { label: '筆數', value: stats.count.toString(), color: 'text-slate-300' },
            { label: '勝率', value: `${stats.winRate.toFixed(0)}%`, color: stats.winRate >= 50 ? 'text-red-400' : 'text-green-400' },
            { label: '總損益', value: `${stats.total >= 0 ? '+' : ''}${stats.total.toLocaleString('zh-TW', { maximumFractionDigits: 0 })}`, color: stats.total >= 0 ? 'text-red-400' : 'text-green-400' },
            { label: 'Avg W/L', value: `${(stats.avgWin / Math.abs(stats.avgLoss)).toFixed(1)}x`, color: stats.avgWin / Math.abs(stats.avgLoss) >= 1.5 ? 'text-sky-400' : 'text-slate-400' },
        ...
        <div className="p-4 border-b border-slate-800 space-y-3 bg-slate-800/20">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <div className="space-y-1">
              <label className="text-[10px] text-slate-500">日期</label>
            ...
            <div className="space-y-1">
              ...
              <input type="text" value={form.symbol} onChange={e => setForm(f => ({ ...f, symbol: e.target.value.toUpperCase() }))}
                placeholder="2330" className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-white focus:outline-none focus:border-sky-500" />
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              <label className="text-[10px] text-slate-500">股數</label>
              <input type="number" value={form.shares} onChange={e => setForm(f => ({ ...f, shares: Number(e.target.value) }))}
                step={1000} className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-white focus:outline-none focus:border-sky-500 tabular-nums" />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-2">
            <div className="space-y-1">
              <label className="text-[10px] text-slate-500">進場價</label>
              <input type="number" value={form.entryPrice || ''} onChange={e => setForm(f => ({ ...f, entryPrice: Number(e.target.value) }))}
                step={0.01} placeholder="0.00" className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-white focus:outline-none focus:border-sky-500 tabular-nums" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] text-slate-500">出場價</label>
              <input type="number" value={form.exitPrice || ''} onChange={e => setForm(f => ({ ...f, exitPrice: Number(e.target.value) }))}
                step={0.01} placeholder="0.00" className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-white focus:outline-none focus:border-sky-500 tabular-nums" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] text-slate-500">出場原因</label>
          ...
          <div className="space-y-1">
            <label className="text-[10px] text-slate-500">備注</label>
            <input type="text" value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))}
              placeholder="交易心得、進場理由..." className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-white focus:outline-none focus:border-sky-500" />
          </div>
      ...
      {entries.length === 0 ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="text-[10px] text-slate-500 uppercase border-b border-slate-700/60 bg-slate-800/40">
            ...
            <tbody>
              {entries.map(e => {
                ...
                const pct = calcPct(e);
                return (
                  <tr key={e.id} className="border-b border-slate-800/40 hover:bg-slate-800/30 transition-colors">
                    <td className="py-2 px-3 text-slate-400">{e.date}</td>
                    <td className="py-2 px-3 font-mono font-bold text-white">{e.symbol}</td>
                    ...
                    <td className="py-2 px-3 text-right tabular-nums text-slate-300">{e.entryPrice}</td>
                    <td className="py-2 px-3 text-right tabular-nums text-slate-300">{e.exitPrice}</td>
                    <td className={`py-2 px-3 text-right tabular-nums font-bold ${pnl >= 0 ? 'text-red-400' : 'text-green-400'}`}>
