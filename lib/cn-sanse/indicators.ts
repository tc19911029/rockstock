...
export function computeSanSeChart(
  ...
  // 黃金交叉/死叉（黃紅兩線互穿）→ 箭頭
  const buy = CROSS(zb4, zb5);          // 黃金交叉（黃上穿紅）
  const sell = CROSS(zb5, zb4);         // 死叉（黃下穿紅）
  // 站上/跌破「線」（價格穿線）→ B/S 字母
  const breakUp = CROSS(C, zhineng);    // 突破智能線（藍 B）
  const breakDn = CROSS(zhineng, C);    // 跌破智能線（藍 S）
  const yrHigh = zb4.map((v, k) => Math.max(v, zb5[k])); // 紅黃帶上緣
  const yrLow = zb4.map((v, k) => Math.min(v, zb5[k]));  // 紅黃帶下緣
  const breakYR = CROSS(C, yrHigh);     // 突破紅黃線（站上整條帶，紅 B）
  const breakDnYR = CROSS(yrLow, C);    // 跌破紅黃線（跌破整條帶，紅 S）

  const mainMarkers: ChartMarker[] = [];
  for (let i = 0; i < n; i++) {
    // ── 站上線 = B 字母：紅=紅黃線、藍=智能線、紫=同時 ──
    const bYR = !!breakYR[i], bSmart = !!breakUp[i];
    ...
    // ── 黃紅金叉/死叉 = 箭頭：紅箭頭向上 / 綠箭頭向下 ──
    if (buy[i]) mainMarkers.push({ time: dates[i], position: 'belowBar', shape: 'arrowUp', color: RED, text: '' });
    if (sell[i]) mainMarkers.push({ time: dates[i], position: 'aboveBar', shape: 'arrowDown', color: GREEN, text: '' });
  }
  ...
  if (breakYR[i]) buySig.push('突破紅黃線（站上紅黃均線帶，紅 B）');
  if (breakUp[i]) buySig.push('突破智能交易線（站上中期支撐，藍 B）');
  if (breakYR[i] && breakUp[i]) buySig.push('同時突破紅黃線＋智能線（紫 B，雙線齊站上）');
