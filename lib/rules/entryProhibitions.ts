...
export function checkLongProhibitions(
  ...
  // ── 戒律5：回檔跌破月線、再上漲未突破月線（書本 p.57） ─────────────────────
  // 2026-05-09 移除 dead code：原本「今日 close < MA20 就擋」的簡化版，
  // 但條件 ③ 已要求 close > MA20，所以這條 100% 永不觸發。
  // 戒律 5 的書本語意已由六條件 ③ 上達涵蓋。
  // ──────────────────────────────────────────────────────────────
  ...
  //
  // 賠少-9 語意分流（2026-07-04 體檢註記）：「量價背離」在書本有兩個用途、方向相反 —
  //   (a) 進場端（這裡，CH5 戒律）：背離＝勿進場做多（禁入條件）
  //   (b) 持倉端（CH4-05）：上漲一段後背離＝停利警訊（該出不該進 ≠ 該砍既有持倉的停損）
  // 兩者刻意分開實作：進場走本檔戒律3（三合一硬禁入），持倉走 sellSignals/trendAnalysis
  // 的量價背離警示（顯示層）。不可互相挪用，也不可把進場禁入邏輯反推成持倉硬出場。
  {
    const kd = last.kdK;
